...
final class MetalShapeAtlasManifestTests: XCTestCase {
    func testManifestHasExactStableInventory() throws {
        ...
        XCTAssertEqual(manifest.shapes.count, 9)
        XCTAssertEqual(manifest.shapes.filter { $0.source == "genome" }.count, 5)
        ...
        XCTAssertEqual(manifest.shapes.filter { $0.morphology == "softClover" }.map(\.id), ["genome.soft-clover"])
