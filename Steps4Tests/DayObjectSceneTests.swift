...
final class GalleryDayObjectsInputAdapterTests: XCTestCase {
    private let paletteCategories: Set<ModernPaletteCategory> = [.pastel, .cold]

    func testMapsSavedDayToDeterministicFullCanvasProductionInput() {
        let ids = (0..<12).map {
            UUID(uuidString: String(format: "00000000-0000-0000-0000-%012d", $0 + 1))!
        }
        let canvas = makeCanvas(dayKey: "2026-08-30", ids: ids)

        let input = GalleryDayObjectsInputAdapter.makeSceneInput(
            dayCanvas: canvas,
            stepsPoints: 10,
            sleepPoints: 5,
            reduceMotion: true,
        ...
        XCTAssertEqual(input.dayKey, "2026-08-30")
        XCTAssertEqual(input.identity, GalleryDayObjectsInputAdapter.productionIdentity)
        XCTAssertEqual(input.eventIDs, ids.prefix(10).map(\.uuidString))
        XCTAssertEqual(input.motionEnergy, 0.5, accuracy: 0.000_001)
        XCTAssertEqual(input.visualClarity, 0.25, accuracy: 0.000_001)
        XCTAssertTrue(input.reduceMotion)
        XCTAssertEqual(input.canvasCoverage, .fullCanvas)
        XCTAssertEqual(input.uiExclusionRegion, .empty)
        XCTAssertEqual(input.paletteCategories, paletteCategories)
    }

    func testHealthInputsClampFromNearStillAndBlurredToFullTempoAndClear() {
        let canvas = makeCanvas(dayKey: "2026-08-30", ids: [])
        let minimum = GalleryDayObjectsInputAdapter.makeSceneInput(
            dayCanvas: canvas,
            stepsPoints: -4,
            sleepPoints: -7,
            reduceMotion: false,
        ...
        let maximum = GalleryDayObjectsInputAdapter.makeSceneInput(
            dayCanvas: canvas,
            stepsPoints: EnergyDefaults.stepsMaxPoints + 8,
            sleepPoints: EnergyDefaults.sleepMaxPoints + 9,
            reduceMotion: false,
            paletteCategories: paletteCategories
        )

        let stillEnvironment = DayObjectEnvironment(
            motionEnergy: minimum.motionEnergy,
            visualClarity: minimum.visualClarity,
            reduceMotion: minimum.reduceMotion
        )
        let fullEnvironment = DayObjectEnvironment(
            motionEnergy: maximum.motionEnergy,
            visualClarity: maximum.visualClarity,
            reduceMotion: maximum.reduceMotion
        )
        let tired = DayObjectPostProcess(
            visualClarity: minimum.visualClarity,
            reduceMotion: minimum.reduceMotion,
            grainSeed: 1
        )
        let rested = DayObjectPostProcess(
            visualClarity: maximum.visualClarity,
            reduceMotion: maximum.reduceMotion,
            grainSeed: 1
        )

        XCTAssertEqual(minimum.motionEnergy, 0)
        XCTAssertEqual(maximum.motionEnergy, 1)
        XCTAssertEqual(stillEnvironment.tempoScale, 0.035, accuracy: 0.000_001)
        XCTAssertEqual(fullEnvironment.tempoScale, 1.25, accuracy: 0.000_001)
        XCTAssertEqual(minimum.visualClarity, 0)
        XCTAssertEqual(maximum.visualClarity, 1)
        XCTAssertGreaterThan(tired.blurRadius, rested.blurRadius)
        XCTAssertEqual(rested.blurRadius, 0, accuracy: 0.000_001)
    }

    func testStableUUIDOrderKeepsRetainedActorsAcrossInsertionAndRemoval() throws {
        let ids = (0..<11).map {
            UUID(uuidString: String(format: "10000000-0000-0000-0000-%012d", $0 + 1))!
        }
        let originalInput = GalleryDayObjectsInputAdapter.makeSceneInput(
            dayCanvas: makeCanvas(dayKey: "2026-08-30", ids: Array(ids.prefix(9))),
            stepsPoints: 10,
            sleepPoints: 10,
            reduceMotion: false,
            paletteCategories: paletteCategories
        )
        let addedInput = GalleryDayObjectsInputAdapter.makeSceneInput(
            dayCanvas: makeCanvas(dayKey: "2026-08-30", ids: Array(ids.prefix(11))),
            stepsPoints: 10,
            sleepPoints: 10,
            reduceMotion: false,
            paletteCategories: paletteCategories
        )
        let removedInput = GalleryDayObjectsInputAdapter.makeSceneInput(
            dayCanvas: makeCanvas(
                dayKey: "2026-08-30",
                ids: ids.enumerated().filter { $0.offset != 3 }.map(\.element)
            ),
            stepsPoints: 10,
            sleepPoints: 10,
            reduceMotion: false,
        ...
        XCTAssertEqual(addedInput.eventIDs.count, DayObjectScene.maxActors)
        XCTAssertEqual(addedInput.eventIDs, ids.prefix(10).map(\.uuidString))

        let original = DayObjectScene.make(input: originalInput)
        let added = DayObjectScene.make(input: addedInput)
        let removed = DayObjectScene.make(input: removedInput)
        for actor in original.actors {
            XCTAssertEqual(try XCTUnwrap(added.actors.first { $0.id == actor.id }), actor)
            if actor.eventID != ids[3].uuidString {
                XCTAssertEqual(try XCTUnwrap(removed.actors.first { $0.id == actor.id }), actor)
            }
        }
    }

    private func makeCanvas(dayKey: String, ids: [UUID]) -> DayCanvas {
        var canvas = DayCanvas(dayKey: dayKey)
        canvas.elements = ids.enumerated().map { index, id in
            CanvasElement(
                id: id,
                kind: .circle,
                optionId: "event-\(index)",
                label: nil,
                hexColor: "#FFFFFF",
                size: 0.2,
