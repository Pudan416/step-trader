def build(style):
 ...
 for rec in f['name'].names:
  names={1:'Nowhere Display 05',2:style,3:f'NowhereDisplay05-{style}-0.500',4:f'Nowhere Display 05 {style}',5:'Version 0.500',6:f'NowhereDisplay05-{style}',16:'Nowhere Display 05',17:style}
  if rec.nameID in names:rec.string=names[rec.nameID].encode(rec.getEncoding())
 f['head'].fontRevision=.5
 for c, contours in OUTLINES.items():
  f['glyf'][cm[ord(c)]]=curve_glyph(contours)
 p=OUT/f'fonts/NowhereDisplay05-{style}.ttf';f.save(p);f.flavor='woff2';f.save(p.with_suffix('.woff2'))
