...
for style in ['Regular','Bold']:
 p=R/f'fonts/NowhereDisplay05-{style}.ttf';f=TTFont(p);base=TTFont(R/f'source/base-{style}.ttf');wf=TTFont(p.with_suffix('.woff2'));cm=f.getBestCmap();order=f.getGlyphOrder()
 ...
 expected={'e':2,'е':2,'ё':4,'g':2,'m':1,'n':1,'h':1,'r':1,'к':1,'G':1,'&':3,'@':2,'f':1,'t':1}
 for c,num in expected.items():
  gl=f['glyf'][cm[ord(c)]];assert gl.numberOfContours==num,(c,gl.numberOfContours)
  if c!='к':assert any(not x&1 for x in gl.getCoordinates(f['glyf'])[2]),('missing curves',c)
