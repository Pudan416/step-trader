def build(style):
 ...
 # Optical spacing for numerical strings; all contexts use the same rule.
 numerical=[ord(c) for c in '0123456789/:.-']
 for a in numerical:
  for b in numerical:
   an,bn=cm[a],cm[b];sa,sb=shapes[an],shapes[bn];gaps=[]
   for y in range(30,701,12):
    ca=sa.intersection(LineString([(-500,y),(2200,y)]));cb=sb.intersection(LineString([(-500,y),(2200,y)]))
    if not ca.is_empty and not cb.is_empty:gaps.append(f['hmtx'][an][0]-ca.bounds[2]+cb.bounds[0])
   if gaps:pairs[(an,bn)]=max(-70,min(70,round((76-min(gaps))/2)*2))
 ...
 for rec in f['name'].names:
  names={1:'Nowhere Display 06',2:style,3:f'NowhereDisplay06-{style}-0.600',4:f'Nowhere Display 06 {style}',5:'Version 0.600',6:f'NowhereDisplay06-{style}',16:'Nowhere Display 06',17:style}
  if rec.nameID in names:rec.string=names[rec.nameID].encode(rec.getEncoding())
 f['head'].fontRevision=.6
 ...
 p=OUT/f'fonts/NowhereDisplay06-{style}.ttf';f.save(p);f.flavor='woff2';f.save(p.with_suffix('.woff2'))
