...
if (( $# < 4 )); then
  print -u2 'usage: run-heavy-in-worktree.sh <label> <zero|nonzero> <working-directory> <command> [args...]'
  exit 64
fi
...
director_root='/Users/kosta/Documents/Documents - Konstantin’s MacBook Pro/dev local/step-trader/.worktrees/editorial-task1b-attempt5-3e5f309/artifacts/day-objects-editorial-field/director/task-1b-3e5f309-attempt5'
...
if [[ ! -d "$command_workdir" ]]; then
  print -u2 "working directory does not exist: $command_workdir"
  exit 66
fi

mkdir -p "$command_root"
cd "$command_workdir" || exit 66
