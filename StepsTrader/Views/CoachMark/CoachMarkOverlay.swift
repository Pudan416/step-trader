        ...
        ZStack {
            ...
            VStack(spacing: 20) {
                Text(manager.tooltip(for: step))
                    .font(.appRounded(17, weight: .light, relativeTo: .body))
                    .italic()
                ...
                HStack(spacing: 16) {
                    Button { manager.skipAll() } label: {
                        Text("skip all")
                            .font(.appRounded(15, weight: .light, relativeTo: .subheadline))
                    ...
                    if step.hasNextButton {
                        Button { manager.advance() } label: {
                            Text(step == .allSet ? "done" : "next")
                                .font(.appRounded(15, weight: .semibold, relativeTo: .subheadline))
...
struct CoachMarkSheetOverlay: View {
    ...
    private func sheetTooltipContent(step: CoachMarkStep) -> some View {
        VStack(spacing: 12) {
            Text(manager.tooltip(for: step))
                .font(.appRounded(15, weight: .light, relativeTo: .body))
                .italic()
                .foregroundStyle(.white.opacity(0.9))
                .multilineTextAlignment(.center)
                .lineSpacing(3)
