    ...
    var body: some View {
        NavigationView {
            Form {
                ...
                Section(header: Text(loc(appLanguage, "Minute Mode Logs", "Логи минутного режима"))) {
                    NavigationLink {
                        MinuteChargeLogsView(model: model, automationApps: Self.automationAppsStatic, appLanguage: appLanguage)
                            .navigationTitle(loc(appLanguage, "Minute Charges", "Списания"))
                            .navigationBarTitleDisplayMode(.inline)
                    } label: {
                        HStack {
                            Text(loc(appLanguage, "View charge logs", "Посмотреть логи"))
                            Spacer()
                            let totalMinutes = model.totalMinutesToday()
                            Text("\(totalMinutes) " + loc(appLanguage, "min today", "мин сегодня"))
                                .foregroundColor(.secondary)
                        }
                    }
                    
                    Button {
                        model.refreshMinuteChargeLogs()
                    } label: {
                        HStack {
                            Image(systemName: "arrow.clockwise")
                            Text(loc(appLanguage, "Refresh logs", "Обновить логи"))
                        }
                    }
                }
...
// MARK: - Minute Charge Logs View
struct MinuteChargeLogsView: View {
    @ObservedObject var model: AppModel
    ...
    private func appName(for bundleId: String) -> String {
        automationApps.first { $0.bundleId == bundleId }?.name ?? bundleId
    }
    
    var body: some View {
        List {
            // Summary section
            Section(header: Text(loc(appLanguage, "Today's Summary", "Сводка за сегодня"))) {
                let dayKey = AppModel.dayKey(for: Date())
                if let todayMap = model.minuteTimeByDay[dayKey], !todayMap.isEmpty {
                    ForEach(todayMap.sorted(by: { $0.value > $1.value }), id: \.key) { bundleId, minutes in
                        HStack {
                            Text(appName(for: bundleId))
                            Spacer()
                            Text("\(minutes) " + loc(appLanguage, "min", "мин"))
                                .foregroundColor(.secondary)
                        }
                    }
                    
                    HStack {
                        Text(loc(appLanguage, "Total", "Всего"))
                            .fontWeight(.semibold)
                        Spacer()
                        Text("\(model.totalMinutesToday()) " + loc(appLanguage, "min", "мин"))
                            .fontWeight(.semibold)
                    }
                } else {
                    Text(loc(appLanguage, "No minute mode usage today", "Нет использования минутного режима сегодня"))
                        .foregroundColor(.secondary)
                }
            }
            
            // Recent charges section
            Section(header: Text(loc(appLanguage, "Recent Charges", "Последние списания"))) {
                if model.minuteChargeLogs.isEmpty {
                    Text(loc(appLanguage, "No charges recorded", "Нет записей"))
                        .foregroundColor(.secondary)
                } else {
                    ForEach(model.minuteChargeLogs.suffix(50).reversed()) { log in
                        VStack(alignment: .leading, spacing: 4) {
                            HStack {
                                Text(appName(for: log.bundleId))
                                    .font(.headline)
                                Spacer()
                                Text("-\(log.cost) " + loc(appLanguage, "steps", "шагов"))
                                    .font(.subheadline)
                                    .foregroundColor(.red)
                            }
                            HStack {
                                Text(dateFormatter.string(from: log.timestamp))
                                    ...
                                    .foregroundColor(.secondary)
                                Spacer()
                                Text(loc(appLanguage, "Balance: ", "Баланс: ") + "\(log.balanceAfter)")
                                    .font(.caption)
                                    .foregroundColor(.secondary)
                            }
                        }
            ...
            Section(header: Text(loc(appLanguage, "History by Day", "История по дням"))) {
                let sortedDays = model.minuteTimeByDay.keys.sorted().reversed()
                ForEach(Array(sortedDays), id: \.self) { dayKey in
                    if let dayMap = model.minuteTimeByDay[dayKey] {
                        let totalMinutes = dayMap.values.reduce(0, +)
                        DisclosureGroup {
                            ForEach(dayMap.sorted(by: { $0.value > $1.value }), id: \.key) { bundleId, minutes in
                                HStack {
                                    Text(appName(for: bundleId))
                                        .font(.subheadline)
                                    Spacer()
                            ...
                            HStack {
                                Text(dayKey)
                                Spacer()
                                Text("\(totalMinutes) " + loc(appLanguage, "min", "мин"))
                                    .foregroundColor(.secondary)
                            }
                        }
                    }
            ...
            Section {
                Button(role: .destructive) {
                    model.clearMinuteChargeLogs()
                } label: {
                    HStack {
                        Image(systemName: "trash")
                        Text(loc(appLanguage, "Clear all logs", "Очистить все логи"))
                    }
                }
            }
        }
