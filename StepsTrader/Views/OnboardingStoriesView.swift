...
struct OnboardingStoriesView: View {
                ...
                VStack(spacing: 8) {
                    if showFeedHint {
                        Text(String(localized: "first you need to add any one app"))
                            .font(.appRounded(13, weight: .light, relativeTo: .footnote))
                    ...
                    if isAppleLoginSkippable {
                        Button(action: next) {
                            Text(skipText)
                                .font(.appRounded(14, weight: .light, relativeTo: .subheadline))
    ...
    private func onboardingLineText(_ line: String, size: CGFloat, relativeTo textStyle: Font.TextStyle = .body) -> some View {
        Text(line)
            .font(.appRounded(size, weight: .light, relativeTo: textStyle))
            ...
            ZStack {
                ...
                if !tappedOrbs.isEmpty {
                    Text("\(tappedOrbs.count * 20)")
                        .font(.appRounded(36, weight: .thin, relativeTo: .largeTitle))
                        .foregroundStyle(accent)
                    ...
                    Button {
                        ...
                        VStack(spacing: 4) {
                            ...
                            Text(String(localized: "+20"))
                                .font(.appRounded(10, weight: .light, relativeTo: .caption))
    ...
    private func spendDemoSlide(slide: OnboardingSlide) -> some View {
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                Text("\(demoColorPool)")
                    .font(.appRounded(28, weight: .thin, relativeTo: .title2))
                    .foregroundStyle(accent)
                    .contentTransition(.numericText())
                Text(String(localized: "colors"))
                    .font(.appRounded(14, weight: .light, relativeTo: .caption))
                    .foregroundStyle(.white.opacity(0.35))
            }
            .padding(.bottom, 20)
            ...
            if !isUnlocked {
                Text(String(localized: "Instagram is closed."))
                    .font(.appRounded(14, weight: .light, relativeTo: .footnote))
            ...
            VStack(spacing: 10) {
                ForEach(Array(tariffs.enumerated()), id: \.offset) { i, tariff in
                    ...
                    Button {
                        ...
                        HStack {
                            Text(tariff.label)
                                .font(.appRounded(16, weight: .light, relativeTo: .body))
            ...
            HStack(spacing: 12) {
                ForEach(Array(steps.enumerated()), id: \.offset) { i, step in
                    ...
                    VStack(spacing: 8) {
                        ...
                        Text(step.label)
                            .font(.appRounded(13, weight: .light, relativeTo: .caption))
    ...
    private func stepsSetupSlide(slide: OnboardingSlide) -> some View {
        VStack(spacing: 0) {
            ...
            Text(formatGroupedNumber(Int(stepsTarget)))
                .font(.appRounded(60, weight: .thin, relativeTo: .largeTitle))
                .foregroundStyle(accent)
                .contentTransition(.numericText())
            
            Text(String(localized: "steps", comment: "Onboarding – steps unit label"))
                .font(.appRounded(16, weight: .light, relativeTo: .subheadline))
                ...
                ForEach(popularApps, id: \.bundleId) { app in
                    ...
                    Button {
                        ...
                        VStack(spacing: 8) {
                            ZStack(alignment: .topTrailing) {
                                ...
                                if isSelected {
                                    Image(systemName: "checkmark.circle.fill")
                                        .font(.appRounded(16, relativeTo: .subheadline))
                                        .foregroundStyle(accent)
                            ...
                            Text(app.name)
                                .font(.appRounded(11, weight: .light, relativeTo: .caption))
            ...
            if let auth = authService, auth.hasAppleAccount {
                HStack(spacing: 10) {
                    Image(systemName: "checkmark.circle")
                        .font(.appRounded(20, weight: .thin, relativeTo: .title3))
                        .foregroundStyle(.green.opacity(0.8))
                    Text(String(localized: "Signed in", comment: "Onboarding – Apple sign-in success state"))
                        .font(.appRounded(18, weight: .light, relativeTo: .body))
                ...
                if let name = authService?.currentUser?.displayName,
                   !name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                    Text(name.trimmingCharacters(in: .whitespacesAndNewlines))
                        .font(.appRounded(36, weight: .thin, relativeTo: .title))
                        .foregroundStyle(accent)
                    ...
                    if bodyMindHeartVisible > i {
                        HStack(spacing: 14) {
                            ...
                            VStack(alignment: .leading, spacing: 3) {
                                Text(cat.label)
                                    .font(.appRounded(16, weight: .medium, relativeTo: .body))
                                    .foregroundStyle(.white.opacity(0.85))
                                Text(cat.desc)
                                    .font(.appRounded(13, weight: .light, relativeTo: .caption))
                                    .foregroundStyle(.white.opacity(0.45))
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 24) {
                Text(String(localized: "welcome to"))
                    .font(.appRounded(20, weight: .light, relativeTo: .title3))
                    .foregroundStyle(.white.opacity(0.5))
                ...
                if welcomeNameVisible,
                   ...
                   !name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                    Text(name.trimmingCharacters(in: .whitespacesAndNewlines))
                        .font(.appRounded(28, weight: .thin, relativeTo: .title))
                        .foregroundStyle(accent.opacity(0.8))
                        .transition(.opacity)
                }
            }
