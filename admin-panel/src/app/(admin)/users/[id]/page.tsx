...
export default async function UserDetailPage({
  ...
  const [shields, shieldCount, prefs] = await Promise.all([
    listShields(id),
    countShields(id),
    getUserPrefs(id),
  ]);
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-950">
          ...
          <div className="mt-2 space-y-1 text-sm">
            ...
            <div>
              ...
              {prefs?.last_opened_at
                ? new Date(prefs.last_opened_at).toLocaleString()
                : "—"}
            </div>
            <div>
              <span className="text-zinc-500">widgets:</span>{" "}
              {[
