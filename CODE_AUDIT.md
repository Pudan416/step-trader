...
- **What:** The observer's completion handler dispatches into an untracked `Task { … }`. If `stopObservingSteps()` runs while a `fetchSteps()` is in flight, the late callback still writes to `lastStepCount` and pushes a stale value through `updateHandler`.
- **Why:** The lock keeps the write atomic, but the value being pushed is from the moment the observer fired, not the moment the user expected observation to end.
- **Action:** Store the in-flight Task; cancel it in `stopObservingSteps()` before nilling `observerQuery`.
- **Severity:** Medium
...
- **What:** Inside `startObservingSteps`, an unguarded Task does `fetchSteps(...)` then `beginObservation(...)`. If `stopObservingSteps()` is invoked between the two awaits, observation still starts.
- **Why:** Leaves a live `HKObserverQuery` after the caller has logically stopped observing, defeating teardown.
- **Action:** Store this Task on the service; cancel it in `stopObservingSteps()`.
- **Severity:** Medium
- **Что это значит на практике:** Технически это вариация §3.6 / §5.3 — лайфтайм HealthKit observer не контролируется. Можно остановить «наблюдение», но fetch уже улетит и UI обновится.
...
- **What:** Cross-reference of §3.9 from a bug-impact angle.
- **Action:** Store and cancel the initial-fetch Task.
- **Severity:** High
- **Что это значит на практике:** sign-out → sign-in flow: предыдущий юзер на секунду может мелькнуть в step count нового аккаунта в момент переключения. Связано с §3.6, §3.9.
