...
**Date:** 2026-08-09
**Status:** Done. The harness has been removed — this file is the record, not a
live runbook. The code that produced the answer is preserved at tag
`spike/feeds-live-activity-archive` (commit `e563f72`) and can be restored with
`git checkout spike/feeds-live-activity-archive`.
**Answers:** `Feeds-Spec.md` § "Resolved: the monitor extension cannot update the
Live Activity" and the first spike checkbox in `Feeds-Brief.md` §6.
